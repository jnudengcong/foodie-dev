...
import com.cong.enums.PaymentStatus;
import com.cong.pojo.Orders;
import com.cong.pojo.bo.MerchantOrdersBO;
import com.cong.pojo.vo.PaymentInfoVO;
import com.cong.resource.AliPayResource;
import com.cong.resource.WXPayResource;
import com.cong.service.PaymentOrderService;
import com.cong.utils.CurrencyUtils;
import com.cong.utils.CONGJSONResult;
import com.cong.utils.RedisOperator;
import com.cong.wx.entity.PreOrderResult;
import com.cong.wx.service.WxOrderService;
...
public class PaymentController {
	...
	public CONGJSONResult createMerchantOrder(@RequestBody MerchantOrdersBO merchantOrdersBO, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isBlank(merchantOrderId)) {
			return CONGJSONResult.errorMsg("参数[orderId]不能为空");
		}
		if (StringUtils.isBlank(merchantUserId)) {
			return CONGJSONResult.errorMsg("参数[userId]不能为空");
		}
		if (amount == null || amount < 1) {
			return CONGJSONResult.errorMsg("参数[realPayAmount]不能为空并且不能小于1");
		}
		if (payMethod == null) {
			return CONGJSONResult.errorMsg("参数[payMethod]不能为空并且不能小于1");
		}
		if (payMethod != PayMethod.WEIXIN.type && payMethod != PayMethod.ALIPAY.type) {
			return CONGJSONResult.errorMsg("参数[payMethod]目前只支持微信支付或支付宝支付");
		}
		if (StringUtils.isBlank(returnUrl)) {
			return CONGJSONResult.errorMsg("参数[returnUrl]不能为空");
		}
		...
		try {
			...
			e.printStackTrace();
			CONGJSONResult.errorException(e.getMessage());
		}

		if (isSuccess) {
			return CONGJSONResult.ok("商户订单创建成功！");
		} else {
			return CONGJSONResult.errorMsg("商户订单创建失败，请重试...");
		}
	}
	...
	public CONGJSONResult getPaymentCenterOrderInfo(String merchantOrderId, String merchantUserId) {

		if (StringUtils.isBlank(merchantOrderId) || StringUtils.isBlank(merchantUserId)) {
			return CONGJSONResult.errorMsg("查询参数不能为空！");
		}

		Orders orderInfo = paymentOrderService.queryOrderInfo(merchantUserId, merchantOrderId);

		return CONGJSONResult.ok(orderInfo);
	}
...
//	@GetMapping(value="/getWXPayQRCode")
	...
	public CONGJSONResult getWXPayQRCode(String merchantOrderId, String merchantUserId) throws Exception{
...
//				String total_fee = "1";	// 测试用 1分钱
			...
			return CONGJSONResult.ok(paymentInfoVO);
		} else {
			return CONGJSONResult.errorMsg("该订单不存在，或已经支付");
		}
	}
	...
	public CONGJSONResult goAlipay(String merchantOrderId, String merchantUserId) throws Exception{
...
//    	String total_amount = "0.01";	// 测试用 1分钱
		...
		return CONGJSONResult.ok(alipayForm);
	}
